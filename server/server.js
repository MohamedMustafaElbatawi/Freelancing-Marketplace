...
const allowedOrigins = [
  ...
  // حط هنا دومين الـ Frontend بعد ما يتعمل Deploy
...
app.get("/api/conversations", authMiddleware, async (req, res) => {
  try {
    const conversations = await Conversation.find({
      ...
      .populate(
        "participants",
        "fullName userName email profilePhoto role professionalTitle",
      )
      .sort({
        lastMessageAt: -1,
        createdAt: -1,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const populatedMessage = await Message.findById(message._id).populate(
        "sender",
        "fullName userName email profilePhoto role",
      );
  ...
  async (req, res) => {
    try {
      ...
      const messages = await Message.find({
        ...
        .populate("sender", "fullName userName email profilePhoto role")
        .sort({
          createdAt: 1,
        });
...
app.get("/", (req, res) => {
  res.status(200).json({
    success: true,
    message: "Freelancing Marketplace API is running",
  });
});
